...
exports.seedDemoScenario = seedDemoScenario;
const index_js_1 = require("../../shared/db/index.js");
const index_js_2 = require("../../modules/intake/index.js");
const index_js_3 = require("../../modules/referral/index.js");
const index_js_4 = require("../../modules/offline-gateway/index.js");
const uttarakhand_js_1 = require("../../modules/pan-india/uttarakhand.js");
function seedDemoScenario() {
  ...
  `);
    ...
    (0, uttarakhand_js_1.seedUttarakhandToDb)(index_js_1.db);
    const now = new Date().toISOString();
    // 1. Seed Users (Roles from ARCHITECTURE §8)
    const insertUser = index_js_1.db.prepare(`
  ...
  `);
    ...
    const insertHospital = index_js_1.db.prepare(`
  ...
  `);
    ...
    insertHospital.run('hosp-a-local', 'Community Health Centre (Hospital A)', 'rural_hospital', JSON.stringify(['general_medicine', 'minor_trauma', 'maternity']), 20, 1, JSON.stringify(['ecg', 'blood_test', 'xray']), JSON.stringify(['paracetamol', 'basic_antibiotics', 'saline']), JSON.stringify({ latitude: 19.8820, longitude: 75.3510, address: 'Nearby Rural Taluka Road, 2km' }), '+91-240-2334001', 'specialist_unavailable', // Amber/Yellow
    now);
    // Hospital B: 9 km from village, HAS Cardiac Cath Lab + Cardiac Specialist on site, 12/40 beds
    insertHospital.run('hosp-b-district', 'District Super-Specialty Medical Centre (Hospital B)', 'district_hospital', JSON.stringify(['cardiac', 'trauma', 'icu', 'neonatal', 'surgery']), 40, 12, JSON.stringify(['ct_scan', 'cath_lab', 'ecg', 'ultrasound', 'blood_bank']), JSON.stringify(['streptokinase', 'antivenom', 'heparin', 'insulin', 'antibiotics']), JSON.stringify({ latitude: 19.9450, longitude: 75.4120, address: 'District Highway Junction, 9km' }), '+91-240-2339999', 'phc_functioning', // Green
    now);
    // Facility C: Primary Health Centre (PHC) Rampur (for referral chain demo)
    insertHospital.run('hosp-phc-rampur', 'Primary Health Centre (PHC Rampur)', 'phc', JSON.stringify(['primary_care', 'immunization', 'basic_maternity']), 6, 4, JSON.stringify(['rapid_malaria_kit', 'glucometer', 'hemoglobinometer']), JSON.stringify(['paracetamol', 'iron_folic_acid', 'oral_rehydration']), JSON.stringify({ latitude: 19.8520, longitude: 75.3120, address: 'Village Rampur Main Gate' }), '+91-240-2331122', 'phc_functioning', // Green
    now);
    // Hospital D: Dehradun Regional Hospital (Uttarakhand Demo)
    insertHospital.run('hosp-d-dehradun', 'Dehradun Regional Hospital', 'district_hospital', JSON.stringify(['trauma', 'icu', 'cardiac', 'respiratory']), 50, 15, JSON.stringify(['ct_scan', 'blood_bank', 'xray']), JSON.stringify(['antibiotics', 'antivenom', 'saline']), JSON.stringify({ latitude: 30.3398, longitude: 78.0644, address: 'Dehradun City Center' }), '+91-135-2334455', 'phc_functioning', now);
    // Hospital E: Doiwala Rural Clinic (Uttarakhand Demo)
    insertHospital.run('hosp-e-doiwala', 'Doiwala Rural Clinic', 'rural_hospital', JSON.stringify(['general_medicine', 'minor_trauma']), 10, 3, JSON.stringify(['blood_test']), JSON.stringify(['paracetamol', 'first_aid']), JSON.stringify({ latitude: 30.1600, longitude: 78.1200, address: 'Doiwala Village' }), '+91-135-2223333', 'specialist_unavailable', now);
    // 3. Specialists on site
    const insertSpecialist = index_js_1.db.prepare(`
  ...
  `);
    ...
    // 4. Seed Ambulances
    const insertAmbulance = index_js_1.db.prepare(`
    INSERT INTO ambulances (id, vehicle_number, capability_tags, current_location, status, assigned_incident_id, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
    insertAmbulance.run('amb-108-als', 'MH-20-AX-1088 (ALS Rapid Response)', JSON.stringify(['ALS', 'cardiac', 'trauma']), JSON.stringify({ latitude: 19.8790, longitude: 75.3480, address: 'Ambulance Stand, Village Outpost' }), 'available', null, now);
    insertAmbulance.run('amb-102-bls', 'MH-20-BY-1022 (Basic Transport)', JSON.stringify(['BLS', 'neonatal']), JSON.stringify({ latitude: 19.9100, longitude: 75.3800, address: 'Taluka Depo' }), 'available', null, now);
    insertAmbulance.run('amb-uk-als', 'UK-07-AL-5000 (Hilly Terrain ALS)', JSON.stringify(['ALS', 'trauma']), JSON.stringify({ latitude: 30.3300, longitude: 78.0700, address: 'Dehradun Ambulance Depot' }), 'available', null, now);
    // 5. Seed Patients
    const insertPatient = index_js_1.db.prepare(`
    INSERT INTO patients (id, name, age_band, high_risk_flags, known_conditions, language_pref, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
    insertPatient.run('pat-maternal-01', 'Kavita Patil', 'adult', JSON.stringify(['maternal']), JSON.stringify(['High blood pressure', 'Gestational diabetes']), 'hi', now);
    insertPatient.run('pat-cardiac-01', 'Bhagwanrao Deshmukh', 'geriatric', JSON.stringify(['chronic']), JSON.stringify(['Hypertension', 'History of angina']), 'mr', now);
    ...
    const emergencyReport = (0, index_js_2.reportEmergency)({
    ...
    const referral1 = (0, index_js_3.createReferral)({
    ...
    const followUp = (0, index_js_3.scheduleFollowUp)({
        patient_id: 'pat-maternal-01',
    ...
    const smsReport = (0, index_js_4.parseInboundSMS)({
        from: '+919876543210',
