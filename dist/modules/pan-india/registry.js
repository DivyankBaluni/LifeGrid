...
exports.loadPanIndiaDataset = loadPanIndiaDataset;
exports.searchPanIndiaHospitals = searchPanIndiaHospitals;
exports.getPanIndiaAmbulances = getPanIndiaAmbulances;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
let PAN_INDIA_HOSPITALS = [];
let PAN_INDIA_AMBULANCES = [];
let isLoaded = false;
function loadPanIndiaDataset() {
    if (isLoaded) {
        return {
    ...
    isLoaded = true;
    console.log(`[LIFEGRID Registry] Indexed ${PAN_INDIA_HOSPITALS.length} Pan-India hospitals and ${PAN_INDIA_AMBULANCES.length} ambulance units.`);
    return {
        hospitalsCount: PAN_INDIA_HOSPITALS.length,
        ambulancesCount: PAN_INDIA_AMBULANCES.length,
    };
}
function searchPanIndiaHospitals(query) {
    if (!isLoaded)
        loadPanIndiaDataset();
    ...
    if (query.search) {
        ...
        results = results.filter((h) => (h.name || '').toLowerCase().includes(s) ||
...
function getPanIndiaAmbulances(query) {
    if (!isLoaded)
        loadPanIndiaDataset();
    ...
    if (query?.district) {
        list = list.filter((a) => (a.district || '').toLowerCase() === query.district.toLowerCase());
    }
    return list;
}
