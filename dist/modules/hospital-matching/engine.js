...
exports.getWeightsForPriority = getWeightsForPriority;
...
function estimateEtaMinutes(distanceKm, isEmergency = true) {
...
function getAllHospitals() {
    const stmt = index_js_1.db.prepare(`SELECT * FROM hospitals`);
    const rows = stmt.all();
    return rows.map((r) => {
        const specStmt = index_js_1.db.prepare(`SELECT * FROM specialists WHERE hospital_id = ?`);
...
function matchBestFitHospital(input) {
    ...
    for (const hospital of hospitals) {
        ...
        const missingTags = input.required_tags.filter((tag) => !hospital.capability_tags.includes(tag.toLowerCase()));
        const hasSpecialistMatch = input.required_tags.some((tag) => hospital.specialists_on_site.some((s) => s.available && s.specialty.toLowerCase().includes(tag.toLowerCase())));
        ...
        let totalScore = capabilityMatch * weights.w_cap +
