...
function evaluateTriage(input) {
    ...
    const inputsMissing = [];
    if (rawText.trim())
        inputsUsed.push(`free_text: "${input.raw_symptoms}"`);
    if (checklist.length > 0)
        inputsUsed.push(`checklist: [${input.checklist_symptoms?.join(', ')}]`);
    if (input.age_band)
        inputsUsed.push(`age_band: ${input.age_band}`);
    if (input.known_conditions?.length)
        inputsUsed.push(`known_conditions: [${input.known_conditions.join(', ')}]`);
    ...
    const signalsFired = [];
    const candidatePriorities = new Set();
    ...
    const trace = {
        triage_id: triageId,
        rule_set_version: exports.RULESET_VERSION,
  ...
  `);
    stmt.run(triageId, input.incident_id || null, exports.RULESET_VERSION, JSON.stringify(inputsUsed), JSON.stringify(inputsMissing), JSON.stringify(signalsFired), score, priority, JSON.stringify(confidenceFlags), 0, null, summary, new Date().toISOString());
...
function getTriageResultById(triageId) {
    const stmt = index_js_1.db.prepare(`SELECT * FROM triage_results WHERE id = ?`);
    const row = stmt.get(triageId);
    if (!row)
        return null;
    return {
        triage_id: row.id,
