...
exports.matchBestFitAmbulance = matchBestFitAmbulance;
...
function getAllAmbulances() {
    const stmt = index_js_1.db.prepare(`SELECT * FROM ambulances`);
    const rows = stmt.all();
    return rows.map((r) => ({
        id: r.id,
...
function matchBestFitAmbulance(input) {
    ...
    const targetTag = input.required_tag || (input.priority === 'P1' ? 'ALS' : 'BLS');
    const evaluations = [];
    const maxDist = 25;
    for (const amb of availableAmbulances) {
        const distKm = (0, engine_js_1.calculateDistanceKm)(input.patient_location, amb.current_location);
        const etaMin = (0, engine_js_1.estimateEtaMinutes)(distKm, true);
        const hasTag = amb.capability_tags.includes(targetTag);
        const capabilityMatch = hasTag ? 1.0 : (amb.capability_tags.includes('ALS') ? 0.9 : 0.4);
        ...
        const totalScore = capabilityMatch * 0.40 + proximityScore * 0.35 + etaScore * 0.25;
        evaluations.push({
            id: amb.id,
    ...
    const selectedAmbulance = availableAmbulances.find((a) => a.id === bestEval.id);
    ...
    const trace = {
        ...
        weights: { w_cap: 0.40, w_capacity: 0.0, w_dist: 0.35, w_eta: 0.25 },
    ...
    const updateAmb = index_js_1.db.prepare(`
    UPDATE ambulances
    SET status = 'dispatched', assigned_incident_id = ?, updated_at = ?
    WHERE id = ?
  `);
    updateAmb.run(input.incident_id, new Date().toISOString(), selectedAmbulance.id);
    // Update incident
    const updateInc = index_js_1.db.prepare(`
    UPDATE incidents
    SET matched_ambulance_id = ?, status = 'dispatched', updated_at = ?
    WHERE id = ?
  `);
    updateInc.run(selectedAmbulance.id, new Date().toISOString(), input.incident_id);
