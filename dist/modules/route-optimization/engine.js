...
exports.getStaticCongestionFactor = getStaticCongestionFactor;
...
function getStaticCongestionFactor() {
...
function calculateOptimizedRoute(origin, destination, priority, incident_id) {
    const distanceKm = (0, engine_js_1.calculateDistanceKm)(origin, destination);
    ...
    if (priority === 'P1' || priority === 'P2') {
        ...
        if (incident_id) {
            index_js_1.db.prepare(`UPDATE incidents SET signal_priority_requested = 1 WHERE id = ?`).run(incident_id);
        }
    }
    ...
    if (priority === 'P1' && (finalEta > 20 || distanceKm > 18)) {
        ...
        if (incident_id) {
            index_js_1.db.prepare(`UPDATE incidents SET drone_dispatch_logged = 1 WHERE id = ?`).run(incident_id);
