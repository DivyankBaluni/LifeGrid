...
export interface PredictiveETAResult {
  ...
  congestion_index: number; // 1.0 = baseline, 1.4 = severe
  congestion_level: 'Low' | 'Normal' | 'Moderate' | 'Heavy';
  topography_penalty_minutes: number;
  signal_priority_discount_minutes: number;
  confidence_interval_minutes: [number, number];
  factors_applied: string[];
}

export class LifegridPredictiveETAModel {
  /**
   ...
   */
  public static predictETA(params: {
    origin: GeoPoint;
    destination: GeoPoint;
    priority: TriagePriority;
    vehicleType?: 'ALS' | 'BLS' | 'MICU' | 'NEO';
    timestamp?: Date;
  }): PredictiveETAResult {
    const directDistance = calculateDistanceKm(params.origin, params.destination);
    ...
    let congestionLevel: PredictiveETAResult['congestion_level'] = 'Normal';
    ...
    const predictedMinutes = Math.max(
      2,
      Math.round(baseMinutes * congestionMultiplier + topographyPenalty - signalDiscount)
    );
