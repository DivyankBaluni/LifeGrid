...
import { GeoPoint, TriagePriority } from '../../shared/models/types.js';

export interface UttarakhandHospital {
  id: string;
  name: string;
  tier: 'phc' | 'rural_hospital' | 'district_hospital';
  ...
  accessibility_status: 'phc_functioning' | 'specialist_unavailable' | 'emergency_unavailable' | 'ambulance_available' | 'referral_required';
  doctors: { name: string; specialty: string; available: boolean }[];
  city: string;
}

export interface UttarakhandAmbulance {
  id: string;
  vehicle_number: string;
  capability_tags: string[];
  current_location: GeoPoint;
  status: 'available' | 'dispatched' | 'en_route' | 'at_scene';
  capacity: number; // 1, 2, or 4 patients
  driver_name: string;
...
export interface PatientTriageProfile {
  patient_id: string;
  name: string;
  ...
  requires_doctor_specialty?: string;
}

export interface MultiDispatchAmbulanceAssignment {
  ambulance: UttarakhandAmbulance;
  passengers: PatientTriageProfile[];
  designated_hospital: UttarakhandHospital;
  hospital_match_score: number;
  hospital_match_reason: string;
  eta_to_incident_min: number;
  eta_to_hospital_min: number;
  total_eta_min: number;
  route_to_scene: [number, number][]; // [lat, lng]
  route_to_hospital: [number, number][]; // [lat, lng]
}
...
export interface MultiDispatchResult {
  ...
  assignments: MultiDispatchAmbulanceAssignment[];
  unassigned_patients: PatientTriageProfile[];
...
export function estimateEtaMinutes(distKm: number, isEmergency = true): number {
...
export function loadUttarakhandHospitalsFromGeoJSON(): UttarakhandHospital[] {
  ...
  const rawData = JSON.parse(fs.readFileSync(geojsonPath, 'utf-8'));
  const features = rawData.features || [];
  const hospitals: UttarakhandHospital[] = [];
  ...
  for (let i = 0; i < features.length; i++) {
    ...
    let tier: 'phc' | 'rural_hospital' | 'district_hospital' = 'rural_hospital';
    let capTags: string[] = ['general_medicine', 'first_aid'];
    let bedCap = 40;
    let icuBeds = 4;
    let diagnostics = ['ecg', 'blood_test', 'xray'];
    let doctors: { name: string; specialty: string; available: boolean }[] = [
...
export function getUttarakhandFleet(): UttarakhandAmbulance[] {
  return [
    {
      id: 'uk-amb-101',
...
export function seedUttarakhandToDb(db: Database) {
  ...
  for (const h of hospitals) {
    insertHospital.run(
      ...
      h.name,
      h.tier,
      JSON.stringify(h.capability_tags),
      h.bed_capacity,
      h.beds_available,
      JSON.stringify(h.diagnostics_available),
      JSON.stringify(h.medicines_available),
      JSON.stringify(h.location),
      h.contact_number,
      h.accessibility_status,
      now,
      h.icu_beds,
      h.available_icu
    );
  ...
  for (const amb of fleet) {
    insertAmbulance.run(
      amb.id,
      amb.vehicle_number,
      JSON.stringify(amb.capability_tags),
      JSON.stringify(amb.current_location),
      amb.status,
      null,
      now,
      amb.capacity,
  ...
  console.log(
    `[Uttarakhand Seed] Successfully populated ${hospitals.length} Uttarakhand hospitals & ${fleet.length} active ambulances.`
  );
  return { hospitalsCount: hospitals.length, fleetCount: fleet.length };
}

// 4. Multi-Ambulance & Multi-Patient Hospital Matching Engine
export async function executeMultiPatientDispatch(input: {
  incident_id?: string;
  incident_location: GeoPoint;
  patient_count: number;
  chief_complaint: string;
  custom_patients?: Partial<PatientTriageProfile>[];
  ...
  const patients: PatientTriageProfile[] = [];
  ...
  const selectedAmbulances: UttarakhandAmbulance[] = [];
  ...
  for (const amb of selectedAmbulances) {
    ...
    const targetSpecialties = Array.from(
      new Set(passengers.map((p) => p.requires_doctor_specialty).filter(Boolean) as string[])
    );

    // Score hospitals
    let bestHospital: UttarakhandHospital = allHospitals[0];
    let bestScore = -1;
    let bestReason = '';
    ...
    for (const hosp of allHospitals) {
      ...
      const hasSpecialist = targetSpecialties.every((spec) =>
        hosp.doctors.some((d) => d.available && d.specialty.toLowerCase().includes(spec.toLowerCase()))
      );
    ...
    if (requiresICU) hospitalBedReservations[bestHospital.id].reserved_icu += 1;
    ...
    const route1 = generateInterpolatedRoadPath(
      [amb.current_location.latitude, amb.current_location.longitude],
      [incidentLoc.latitude, incidentLoc.longitude]
    );

    const route2 = generateInterpolatedRoadPath(
      [incidentLoc.latitude, incidentLoc.longitude],
      [bestHospital.location.latitude, bestHospital.location.longitude]
    );
  ...
  await Promise.all(
    assignments.map(async (asgn) => {
      const startAmb: [number, number] = [asgn.ambulance.current_location.latitude, asgn.ambulance.current_location.longitude];
      const sceneLoc: [number, number] = [incidentLoc.latitude, incidentLoc.longitude];
      const hospLoc: [number, number] = [asgn.designated_hospital.location.latitude, asgn.designated_hospital.location.longitude];
  ...
  const result: MultiDispatchResult = {
...
const osrmRouteCache = new Map<string, [number, number][]>();
    ...
    if (res.ok) {
      ...
      if (data.routes && data.routes[0] && data.routes[0].geometry?.coordinates?.length > 1) {
        ...
        const roadPoints: [number, number][] = data.routes[0].geometry.coordinates.map(
          (c: [number, number]) => [Number(c[1].toFixed(5)), Number(c[0].toFixed(5))]
        );
