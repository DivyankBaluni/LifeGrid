...
export function estimateEtaMinutes(distKm, isEmergency = true) {
...
export function loadUttarakhandHospitalsFromGeoJSON() {
...
export function getUttarakhandFleet() {
  return [
    {
      id: 'uk-amb-101',
...
export function seedUttarakhandToDb(db) {
...
export async function executeMultiPatientDispatch(input) {
  ...
  for (const amb of selectedAmbulances) {
    ...
    const targetSpecialties = Array.from(
      new Set(passengers.map((p) => p.requires_doctor_specialty).filter(Boolean))
    );
...
export async function fetchOSRMRouteWithFallback(start, end) {
    ...
    if (res.ok) {
      ...
      if (data.routes && data.routes[0] && data.routes[0].geometry?.coordinates?.length > 1) {
        const roadPoints = data.routes[0].geometry.coordinates.map(
          (c) => [Number(c[1].toFixed(5)), Number(c[0].toFixed(5))]
        );
