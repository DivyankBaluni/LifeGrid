...
import { calculateDistanceKm, estimateEtaMinutes } from '../hospital-matching/engine.js';
...
export interface MatchAmbulanceInput {
  incident_id: string;
  patient_location: GeoPoint;
  priority: TriagePriority;
  required_tag?: 'BLS' | 'ALS' | 'trauma' | 'neonatal';
  actor_user_id?: string | null;
}

export function matchBestFitAmbulance(input: MatchAmbulanceInput): {
  ...
  for (const amb of availableAmbulances) {
    const distKm = calculateDistanceKm(input.patient_location, amb.current_location);
