...
import { AuditLogEntry, ExplainabilityTrace, MatchExplainabilityTrace } from '../models/types.js';
...
export function getAuditLogsForTarget(target_type: string, target_id: string): AuditLogEntry[] {
  const stmt = db.prepare(`
    SELECT * FROM audit_logs
    WHERE target_type = ? AND target_id = ?
    ORDER BY timestamp DESC
  `);
  const rows = stmt.all(target_type, target_id) as any[];
  return rows.map((r) => ({
    id: r.id,
...
export function getAllAuditLogs(limit = 100): AuditLogEntry[] {
  const stmt = db.prepare(`
    SELECT * FROM audit_logs
    ORDER BY timestamp DESC
    LIMIT ?
